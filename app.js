...
app.get('/drivers',function(request,response){
    ergast.getDrivers('2017',function(request,driverList){
...
app.get('/getLapChartData/:seasonid/:round',function(request,response){
    ...
    if (fetchStatic){
        var responseData;
        setTimeout(function(){
            fs.readFile(__dirname+'/data/'+season+'/'+round+'.json', function (err, data) {
                if (err){
                    throw err;
                }
    ...
    function convertToChartData(data){
        /**
         * 
         * 1 - Create lapCount field.
         * 2 - Create laps field - [{"name":Driver Name,
        ...
        pitStopsData = getPitStopsInfo(data.racePitStops.value,data.raceDrivers.value.drivers,lapsData);
        finalData = setRaceStatus(data.raceResults.value.driverResults,pitStopsData);
    ...
    function getLapsData(raceResults,result){
        ...
        raceResults.forEach(function(driverResult,i){
            ...
            driverObj.name = driverResult.driver.givenName+' '+driverResult.driver.familyName;
            driverObj.placing = [];
    ...
    function setRaceStatus(raceResults,result){
        raceResults.forEach(function(raceResult){
            result.laps.find(function(driverData,i){
                if(driverData.driverId === raceResult.driver.driverId && MECHANICAL_PROBLEMS_STATUSES.indexOf(raceResult.status) !== -1){
                    result.laps[i].mechanical.push(raceResult.laps);
                }
                if(driverData.driverId === raceResult.driver.driverId && ACCIDENT_STATUSES.indexOf(raceResult.status) !== -1){
                    result.laps[i].accident.push(raceResult.laps);
                }
                if(driverData.driverId === raceResult.driver.driverId && raceResult.status === 'Disqualified'){
